...
async function main() {
  ...
  if (!address || !address.startsWith("0x") || address.length !== 42) {
    ...
    console.log("  方法2: npm run verify:direct -- <合约地址> \"My NFT Collection\" \"MNFT\" （推荐，最简单）");
    console.log("  方法3: npx hardhat verify --network sepolia <合约地址> \"My NFT Collection\" \"MNFT\"");
    console.log("\n💡 示例:");
    console.log('  # 在 .env 文件中设置:');
    console.log('  CONTRACT_ADDRESS=0x1234567890123456789012345678901234567890');
    console.log('  NFT_NAME=My NFT Collection');
    console.log('  NFT_SYMBOL=MNFT');
    console.log('  npm run verify:contract');
    console.log('');
    console.log('  # 或使用 verify:direct（推荐）:');
    console.log('  npm run verify:direct -- 0x1234567890123456789012345678901234567890 "My NFT Collection" "MNFT"');
    console.log('');
    console.log('  # 或直接使用 Hardhat:');
    console.log('  npx hardhat verify --network sepolia 0x1234567890123456789012345678901234567890 "My NFT Collection" "MNFT"');
    console.log('\n⚠️  重要提示:');
    console.log('  - hardhat run 命令不支持位置参数，必须使用环境变量或 verify:direct');
    console.log('  - MyNFT 合约需要构造函数参数: name symbol');
    process.exit(1);
  }
  ...
  const constructorArguments = [nftName, nftSymbol];
