...
async function main() {
  ...
  console.log("✅ IntegerToRoman 部署成功!");
  console.log("📍 合约地址:", integerToRomanAddress);
  console.log("🔗 Etherscan:", `https://sepolia.etherscan.io/address/${integerToRomanAddress}`);

  // 等待交易确认
  console.log("\n⏳ 等待交易确认...");
  await integerToRoman.deploymentTransaction()?.wait(5);
  console.log("✅ 交易已确认!\n");
  ...
  if (process.env.ETHERSCAN_API_KEY) {
    ...
    try {
      await hre.run("verify:verify", {
        address: integerToRomanAddress,
        constructorArguments: [],
      });
      console.log("✅ IntegerToRoman 合约验证成功!");
    } catch (error) {
      if (error.message.includes("Already Verified")) {
        console.log("✅ IntegerToRoman 合约已验证过");
      } else {
        console.log("⚠️  IntegerToRoman 合约验证失败:", error.message);
      }
    }
  } else {
