...
async function main() {
  ...
  console.log("✅ RomanToInteger 部署成功!");
  console.log("📍 合约地址:", romanToIntegerAddress);
  console.log("🔗 Etherscan:", `https://sepolia.etherscan.io/address/${romanToIntegerAddress}`);

  // 等待交易确认
  console.log("\n⏳ 等待交易确认...");
  await romanToInteger.deploymentTransaction()?.wait(5);
  console.log("✅ 交易已确认!\n");
  ...
  if (process.env.ETHERSCAN_API_KEY) {
    ...
    try {
      await hre.run("verify:verify", {
        address: romanToIntegerAddress,
        constructorArguments: [],
      });
      console.log("✅ RomanToInteger 合约验证成功!");
    } catch (error) {
      if (error.message.includes("Already Verified")) {
        console.log("✅ RomanToInteger 合约已验证过");
      } else {
        console.log("⚠️  RomanToInteger 合约验证失败:", error.message);
      }
    }
  } else {
